...
const isApplied = ref(false); // tracks if discount has been applied
...
watch(
    ...
    (newName) => {
        ...
        // Restore applied state if discount was already saved on this invoice
...
const customerCount = computed(() => 
    props.invoiceDoc?.custom_customer_count || 1
);
...
const totalDiscount = computed(() => {
    ...
    return Math.round(
        (grandTotal.value / customerCount.value) * specialDiscountPercent.value
    );
});
...
// Sync from invoice doc on load
watch(
    () => props.invoiceDoc?.custom_special_discount_type,
    (val) => { selectedDiscountType.value = val || ""; },
    { immediate: true }
);
watch(
    () => props.invoiceDoc?.custom_special_discount_name,
    (val) => { discountName.value = val || ""; },
    { immediate: true }
);
watch(
    () => props.invoiceDoc?.custom_special_discount_id_number,
    (val) => { discountID.value = val || ""; },
    { immediate: true }
);

// Only syncs metadata fields — does NOT apply discount to totals
// Totals are only updated on explicit Save/Clear
...
const handleSave = () => {
    ...
    // Rebalance payment lines to match new grand_total
...
// Clear all discount fields and restore original totals
const handleClear = () => {
    ...
    // Restore payment lines to original grand_total
    const payments = Array.isArray(currentDoc?.payments)
        ? currentDoc.payments.map(p => ({ ...p }))
        : [];
...
// Only sync text fields while typing — don't touch totals
